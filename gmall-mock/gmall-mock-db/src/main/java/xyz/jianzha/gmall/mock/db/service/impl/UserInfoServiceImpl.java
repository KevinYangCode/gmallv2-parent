...
import xyz.jianzha.gmall.mock.db.service.UserInfoService;
import xyz.jianzha.gmall.mock.db.util.*;
...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserInfoService {
    ...
    public UserInfo initUserInfo(Date date) {
        ...
        userInfo.setGender(new RandomOptionGroup(new RanOpt<>("M", maleRateWeight),
                new RanOpt<>("F", 100 - maleRateWeight)).getRandStringValue());
        ...
        userInfo.setUserLevel(new RandomOptionGroup(new RanOpt<>("1", 7),
                new RanOpt<>("2", 2), new RanOpt<>("3", 1)).getRandStringValue());
    ...
    public void updateUsers(Date date) {
        ...
        for (UserInfo userInfo : userInfoList) {
            ...
            if (randInt % 3 == 0) {
                userInfo.setUserLevel(new RandomOptionGroup(new RanOpt<>("1", 7), new RanOpt<>("2", 2),
                        new RanOpt<>("3", 1)).getRandStringValue());
            }
            if (randInt % 5 == 0) {
                String email = RandomEmail.getEmail(6, 12);
                userInfo.setEmail(email);
